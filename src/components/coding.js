...
import { useTheme, useMediaQuery, Grid, Card, CardContent, CardActions, Avatar } from "@mui/material";
...
export default function PinPage() {
                ...
                if (pData && !canceled) {
                    setPin({
                        ...
                        addedBy: pData.addedBy
                            ? { username: pData.addedBy.Username || pData.addedBy.full_name, avatarUrl: pData.addedBy.avatar_url }
                            : null,
                    });
                }
            } catch (e) {
    ...
    const images = [];
    if (pin.MainImage) images.push(pin.MainImage);
    ...
    return (
        <DashboardLayout>
                ...
                <Box sx={{ position: "relative", width: "100vw", left: "50%", transform: "translateX(-50%)" }}>
                    ...
                    >
                        <PinStats
                            ...
                            isWant={isWantToGo}
                            isSaved={isSaved}
                            iconOnly={true}     // <-- hides text & counts
                        />
                    </Box>
                </Box>
            )}
            ...
            >
                ...
                >
                    <CardContent>
                        <Box
                            sx={{
                                // center contents on xs/mobile, left‑align on md+
                                display: "flex",
                        ...
                        >
                            ...
                            <Grid item>
                                <Grid container alignItems="center" spacing={1}>
                                    <Grid item>
                                        <Avatar src={pin.addedBy?.avatarUrl} sx={{ width: 58, height: 58, "& img": { ObjectFit: "cover", wdith: "100%", height: "100%" } }} />
                                    </Grid>
                                    <Grid item>
            ...
            <MDBox mt={4} px={2}>
                <Grid container spacing={3}>
                    <Grid item xs={12} md={8} order={{ xs: 2, md: 1 }}>
                        <div
                            dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(pin.Information || '') }}
                        />
                        <PinInfoEditor
                    ...
                    <Grid item xs={12} md={4} order={{ xs: 1, md: 2 }} container direction="column" spacing={2}>
                        <Grid item>
                            <PinMapCard pin={pin} />
                        </Grid>
                        <Grid item>
            ...
            <ListDialog
              ...
              pin={pin}
              onSaved={() => {
                // toggle in SavedPinsContext
                save(pin);
                // bump the counter in your UI
