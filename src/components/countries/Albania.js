...
function Albania({ countryName }) {
    ...
    const fetchWeather = async () => {
      try {
        const response = await fetch(
          `https://api.openweathermap.org/data/2.5/weather?q=Tirana,AL&units=metric&appid=${apiKey}`
        );
  ...
  const fetchAllPins = async () => {
    let query = supabase.from("pins").select("*").eq("countryName", "Albania");
