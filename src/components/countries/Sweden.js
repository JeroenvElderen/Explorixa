...
function PinCardWithTimeAgo({ pin, idx, truncateDescription, isExpanded }) {
  ...
  const cardHeight = isExpanded ? "800px" : "150px";
  return (
    <PinCard
...
function Sweden({ countryName }) {
  ...
  const parentCountryName = "Sweden";
  ...
  const fetchWeather = async () => {
    try {
      const response = await fetch(
        `https://api.openweathermap.org/data/2.5/weather?q=stockholm,SE&units=metric&appid=${apiKey}`
      );
  ...
  const fetchAllPins = async () => {
    let query = supabase.from("pins").select("*").eq("countryName", "Sweden");
              ...
              {allPins.length === 0 ? (
                ...
                allPins.map((pin, idx) => (
                  <Grid item xs={12} md={6} lg={4} key={pin.id || idx}>
                    ...
                    >
                      <PinCardWithTimeAgo pin={pin} idx={idx} truncateDescription={true} isExpanded={expandedPinId === pin.id}/>
                    </MDBox>
                  </Grid>
                ))
              )}
