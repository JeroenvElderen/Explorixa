...
export default function CountryPage() {
  ...
  const handlePinClick = (pin) => {
    ...
    navigate(
      `/Destinations/${encodeURIComponent(continentParam)}/${encodeURIComponent(
        countrySlug
      )}/${pinSlug}`,
      {
        state: { pin },
  ...
  useEffect(() => {
    supabase
      ...
      .eq("Country", countryName)
      .then(({ data }) =>
        data && setCountryCities(["All", ...new Set(data.map((c) => c.Name))])
      );
    ...
    supabase
      ...
      .select("Category")
      .then(({ data }) =>
        data && setCategories(["All", ...new Set(data.map((c) => c.name))])
      );
  }, [countryName]);
