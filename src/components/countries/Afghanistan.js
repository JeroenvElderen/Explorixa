...
function Afghanistan({ countryName }) {
  ...
  const parentCountryName = "Afghanistan";
  ...
  // Check if the new pin matches current filters before adding
  const matchesCity = selectedCity === "All" || newPin.City === selectedCity;
  const matchesCategory = selectedCategory === "All" || newPin.Category === selectedCategory;

  if (matchesCity && matchesCategory) {
    setAllPins((prev) => [newPin, ...prev]);
  }

  setRecentPins((prev) => [newPin, ...prev].slice(0, 3));
    setPinCount((prevCount) => prevCount + 1);
};

  const handleCancelPinForm = () => {
  setShowPinForm(false);
  setShowAllPins(true); // or whatever makes sense, e.g. show the pin list again
  fetchAllPins();
};
  ...
  useEffect(() => {
    const fetchCitieCount = async () => {
      const { count, error } = await supabase
        .from("cities")
        .select("*", { count: "exact", head: true })
        .eq("Country", "Afghanistan");
      if (!error) setCityCount(count);
    };
    fetchCitieCount();
  }, []);
  ...
  const fetchPopulation = async () => {
    try {
      const response = await fetch("https://restcountries.com/v3.1/name/Afghanistan");
      const data = await response.json();

      if (data && Array.isArray(data) && data.length > 0 && data[0]?.population) {
  setPopulation(data[0].population);
} else {
  console.warn("Unexpected population data format:", data);
}
    } catch (error) {
  ...
  const fetchRecentPins = async () => {
    try {
      ...
      if (error) {
        ...
        setLastPinCreatedTimeAgo("No pins");
        return;
      }
      ...
      console.error("Unexpected error fetching recent pins:", err);
      setLastPinCreatedTimeAgo("No pins");
    }
  };

  fetchRecentPins();
  supabase.getChannels().forEach((ch) => console.log("Active channel:", ch.topic));
  ...
  useEffect(() => {
    const fetchCountryCities = async () => {
      const { data, error } = await supabase
        ...
        .select("Name")
        .eq("Country", "Afghanistan");
      if (!error && Array.isArray(data)) setCountryCities(data.map((city) => city.Name));
    };
    fetchCountryCities();
  }, []);

  const fetchAllPins = async () => {
    let query = supabase.from("pins").select("*").eq("countryName", "Afghanistan");
...
if (Array.isArray(categoryData)) {
  ...
  useEffect(() => {
   ...
   useEffect(() => {
  // Pins subscription for real-time updates
  const pinsChannel = supabase
    .channel('public:pins')
    .on(
      'postgres_changes',
      { event: '*', schema: 'public', table: 'pins', filter: 'countryName=eq.Afghanistan' },
      (payload) => {
        console.log('Realtime payload (pins):', JSON.stringify(payload, null, 2));
        ...
        switch (payload.event) {
          ...
          case 'UPDATE':
            setAllPins((prev) =>
              prev.map((pin) => (pin.id === newPin.id ? newPin : pin))
            );
            setRecentPins((prev) =>
              prev.map((pin) => (pin.id === newPin.id ? newPin : pin))
            );
            break;
          case 'DELETE':
            setAllPins((prev) => prev.filter((pin) => pin.id !== oldPin.id));
            setRecentPins((prev) => prev.filter((pin) => pin.id !== oldPin.id));
            setPinCount((prevCount) => (prevCount > 0 ? prevCount - 1 : 0));
            break;
          default:
            break;
        }
  ...
  // Cities subscription for real-time updates
  const citiesChannel = supabase
    .channel('public:cities')
    .on(
      'postgres_changes',
      { event: '*', schema: 'public', table: 'cities', filter: 'Country=eq.Afghanistan' },
      (payload) => {
        console.log('Realtime payload (cities):', payload);
        ...
        switch (event) {
          case 'INSERT':
            ...
            break;
          case 'DELETE':
            setCityCount((prevCount) => (prevCount > 0 ? prevCount - 1 : 0));
            break;
          default:
            break;
        }
  ...
  // Cleanup function to unsubscribe channels when component unmounts
  return () => {
  ...
  return (
    <DashboardLayout>
      ...
      <MDBox py={3}>
            ...
            <MDBox mb={1.5}>
              ...
              >
                <ComplexStatisticsCard
                  ...
                  icon="place"
                  title="See all pins"
                  count={pinCount !== undefined ? pinCount : "Loading..."}
                  percentage={{
                    color: "success",
          ...
          <Grid item xs={12} md={6} lg={3}>
            <MDBox mb={1.5}>
              <ComplexStatisticsCard
                ...
                count={cityCount !== undefined ? cityCount : "Loading..."}
                percentage={{
                  color: "success",
          ...
          <Grid item xs={12} md={6} lg={3}>
            <MDBox mb={1.5}>
              <ComplexStatisticsCard
                ...
                title="Population"
                count={population?.toLocaleString("en-US") ?? "Loading..."}
                percentage={{
                  color: "success",
                  amount: "",
                  label: "Just updated",
        ...
        {!showAllPins && !showPinForm &&(
          <MDBox>
            <Grid container spacing={3}>
              <Grid item xs={12} md={6} lg={8}>
                <Projects country="Afghanistan" />
              </Grid>
      ...
      {showPinForm && (
        <MDBox mt={4}>
         <PinCardForm
  countryName={parentCountryName}
  onSubmit={handlePinSaved}
